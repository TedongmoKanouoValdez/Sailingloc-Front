...
const ReservationCell = ({ row }: { row: Row<z.infer<typeof schema>> }) => {
  const [showReservation, setShowReservation] = React.useState(false);

  return (
    <>
      <Button onClick={() => setShowReservation(true)}>{row.original.header}</Button>
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-32">
          <DropdownMenuItem onClick={() => setShowReservation(true)}>Lire</DropdownMenuItem>
...
const columns: ColumnDef<z.infer<typeof schema>>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => <ReservationCell row={row} />,
    enableHiding: false,
  },
  {
    accessorKey: 'Aperçudumessage',
    header: 'Aperçu du message',
    cell: ({ row }) => (
      <div className="w-32">
  ...
  {
    accessorKey: 'Typedemessage',
    header: () => <div className="w-full">Type de message</div>,
    cell: ({ row }) => (
      <form
        onSubmit={(e) => {
          e.preventDefault();
